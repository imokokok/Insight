...
test.describe('Oracle Data Display', () => {
  ...
  test('should handle symbol selection', async ({ page }) => {
    ...
    const symbolSelector = page.locator('select, [role="combobox"], button:has-text("BTC"), button:has-text("ETH")').first();
    
    if (await symbolSelector.isVisible().catch(() => false)) {
      await symbolSelector.click();
      
      // Select a different symbol
      const option = page.locator('option:has-text("ETH"), [role="option"]:has-text("ETH")').first();
