-- Retire the Switchboard provider from active platform state.
--
-- Historical price, reputation-history, and snapshot rows are intentionally
-- retained as immutable audit evidence. Application discovery and consensus
-- are driven by oracle_feeds, so deleting those registrations prevents any
-- retired-provider data from participating in new reads.

BEGIN;

DELETE FROM public.oracle_feeds
WHERE provider = 'switchboard';

DELETE FROM public.oracle_reputation
WHERE provider = 'switchboard';
