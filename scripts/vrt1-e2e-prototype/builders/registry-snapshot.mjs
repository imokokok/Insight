...
//
// A key registry snapshot is a small record, it changes rarely, and anchoring it
// turns a key history into something a verifier can check against Bitcoin
// instead of against a web server — a clean second record type alongside the
// safety check.
//
...
//
// action_type: key_registry_snapshot          (GENERIC per VRT1 §8.5:
...
const record = {
  // §8.5 note: schema_version inside the signed payload is
  ...
  draft: true,
  note: 'VRT1 §8.5 key_registry_snapshot (generic, credited to Insight). Integer Unix timestamps + key_type + custody per §8.5. Interop: action_id 39f0508bb57fef962bb9bfb9923ffc220b456597443161a5ea633888a388ce83 (700B) matches the shared conformance registration_candidate.',
};
