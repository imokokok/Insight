...
const base = process.argv[2] || '/Users/imokokok/Documents/insight/scripts/vrt1-e2e-prototype/conformance-round2';
const vecDir = join(base, 'conformance-vectors');

// VRT1 tagged_hash
function taggedHash(tag, data) {
  const th = createHash('sha256').update(tag, 'utf8').digest();
  return createHash('sha256').update(Buffer.concat([th, th, data])).digest('hex');
}
...
check('interop draft action_id', actionId(interop.draft_as_sent.canonical_bytes_hex), interop.draft_as_sent.action_id_hex);
check('interop draft len 636', interop.draft_as_sent.canonical_bytes_len, 636);
check('interop candidate action_id', actionId(interop.registration_candidate.canonical_bytes_hex), interop.registration_candidate.action_id_hex);
check('interop candidate len 700', interop.registration_candidate.canonical_bytes_len, 700);
check('candidate = 39f0508b…', interop.registration_candidate.action_id_hex.slice(0,8), '39f0508b');
...
for (const c of neg.cases) check(`neg: ${c.case.slice(0, 50)}`, actionId(c.canonical_bytes_hex), c.action_id_hex);
for (const c of neg.retained_from_round_1) check(`retained: ${c.case.slice(0, 50)}`, actionId(c.canonical_bytes_hex), c.action_id_hex);
...
for (const p of design.positives) check(`registry genesis/successor: ${p.action_id_hex.slice(0,8)}`, actionId(p.canonical_bytes_hex), p.action_id_hex);
for (const n of design.negatives) check(`registry neg: ${n.case.slice(0, 46)}`, actionId(n.canonical_bytes_hex), n.action_id_hex);

// 4. 我方 registry-snapshot.json（700B 重建版）vs interop registration_candidate
const mine = JSON.parse(readFileSync('/Users/imokokok/Documents/insight/scripts/vrt1-e2e-prototype/registry-snapshot.json', 'utf8'));
...
check('我方 700B action_id', mine.action_id_hex, '39f0508bb57fef962bb9bfb9923ffc220b456597443161a5ea633888a388ce83');
...
const posAct = JSON.parse(
  readFileSync('/Users/imokokok/Documents/insight/scripts/vrt1-e2e-prototype/insight-vectors/action.json', 'utf8')
);
for (const c of neg.cases) {
  check(`我方 canonical ≠ 负向量(${c.case.slice(0, 34)})`, posAct.canonical_bytes_hex !== c.canonical_bytes_hex, true);
}
for (const c of neg.retained_from_round_1) {
  check(`我方 canonical ≠ retained conjunction`, posAct.canonical_bytes_hex !== c.canonical_bytes_hex, true);
}
