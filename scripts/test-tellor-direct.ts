/**
 * 直接测试Tellor合约调用
 */

import {
  getTellorOracleAddress,
  getTellorPriceQuery,
  TELLOR_ORACLE_ABI,
} from '../src/lib/oracles/tellorDataSources';
import { encodeFunctionData } from 'viem';

const RPC_URL = 'https://eth.drpc.org';

async function testTellorContract() {
  console.log('=== 直接测试Tellor合约调用 ===\n');

  const chainId = 1;
  const symbol = 'BTC';

  const oracleAddress = getTellorOracleAddress(chainId);
  const priceQuery = getTellorPriceQuery(symbol);

  console.log('配置信息:');
  console.log(`  链ID: ${chainId}`);
  console.log(`  代币: ${symbol}`);
  console.log(`  预言机地址: ${oracleAddress}`);
  console.log(`  Query ID: ${priceQuery?.queryId}`);
  console.log(`  Asset: ${priceQuery?.asset}`);
  console.log(`  Currency: ${priceQuery?.currency}`);

  if (!oracleAddress || !priceQuery) {
    console.log('\n❌ 配置缺失');
    return;
  }

  // 编码getCurrentValue调用
  const callData = encodeFunctionData({
    abi: TELLOR_ORACLE_ABI,
    functionName: 'getCurrentValue',
    args: [priceQuery.queryId as `0x${string}`],
  });

  console.log(`\n调用数据: ${callData}`);

  try {
    console.log(`\n发送请求到 ${RPC_URL}...`);

    const response = await fetch(RPC_URL, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        jsonrpc: '2.0',
        id: 1,
        method: 'eth_call',
        params: [
          {
            to: oracleAddress,
            data: callData,
          },
          'latest',
        ],
      }),
    });

    console.log(`HTTP状态: ${response.status}`);

    if (!response.ok) {
      console.log(`❌ HTTP错误: ${response.statusText}`);
      return;
    }

    const result = await response.json();
    console.log('\n响应结果:', JSON.stringify(result, null, 2));

    if (result.error) {
      console.log(`❌ RPC错误: ${result.error.message}`);
      return;
    }

    // 解码返回值
    const rawData = result.result as string;
    console.log(`\n原始返回数据: ${rawData}`);

    // 解析返回值: (bytes memory _value, uint256 _timestamp)
    const cleanData = rawData.slice(2); // 去掉0x前缀

    // bytes是动态类型，解析偏移量
    const offset = parseInt(cleanData.slice(0, 64), 16) * 2;
    const timestamp = BigInt('0x' + cleanData.slice(64, 128));

    console.log(`偏移量: ${offset}`);
    console.log(`时间戳: ${timestamp.toString()}`);

    if (timestamp === BigInt(0)) {
      console.log('\n⚠️ 时间戳为0，表示该Query ID没有数据');
      console.log('可能的原因:');
      console.log('  1. Query ID不正确');
      console.log('  2. Tellor预言机上没有该数据');
      console.log('  3. 数据已过期');
      return;
    }

    // 解析bytes数据
    const bytesLength = parseInt(cleanData.slice(offset, offset + 64), 16);
    const bytesData = cleanData.slice(offset + 64, offset + 64 + bytesLength * 2);

    console.log(`Bytes长度: ${bytesLength}`);
    console.log(`Bytes数据: ${bytesData}`);

    // 解析价格 (uint256, 18位小数)
    const rawPrice = BigInt('0x' + bytesData.slice(0, 64));
    const price = Number(rawPrice) / 1e18;

    console.log(`\n✅ 成功获取价格!`);
    console.log(`  原始价格值: ${rawPrice.toString()}`);
    console.log(`  价格: $${price.toLocaleString()}`);
    console.log(`  时间戳: ${new Date(Number(timestamp) * 1000).toISOString()}`);
  } catch (error) {
    console.log(`\n❌ 请求失败: ${error instanceof Error ? error.message : String(error)}`);
  }
}

// 也测试ETH价格
async function testETHPrice() {
  console.log('\n\n=== 测试ETH价格 ===\n');

  const chainId = 1;
  const symbol = 'ETH';

  const oracleAddress = getTellorOracleAddress(chainId);
  const priceQuery = getTellorPriceQuery(symbol);

  if (!oracleAddress || !priceQuery) {
    console.log('❌ 配置缺失');
    return;
  }

  const callData = encodeFunctionData({
    abi: TELLOR_ORACLE_ABI,
    functionName: 'getCurrentValue',
    args: [priceQuery.queryId as `0x${string}`],
  });

  try {
    const response = await fetch(RPC_URL, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        jsonrpc: '2.0',
        id: 1,
        method: 'eth_call',
        params: [
          {
            to: oracleAddress,
            data: callData,
          },
          'latest',
        ],
      }),
    });

    const result = await response.json();

    if (result.error) {
      console.log(`❌ RPC错误: ${result.error.message}`);
      return;
    }

    const rawData = result.result as string;
    const cleanData = rawData.slice(2);
    const timestamp = BigInt('0x' + cleanData.slice(64, 128));

    if (timestamp === BigInt(0)) {
      console.log('⚠️ ETH也没有数据');
      return;
    }

    const offset = parseInt(cleanData.slice(0, 64), 16) * 2;
    const bytesLength = parseInt(cleanData.slice(offset, offset + 64), 16);
    const bytesData = cleanData.slice(offset + 64, offset + 64 + bytesLength * 2);
    const rawPrice = BigInt('0x' + bytesData.slice(0, 64));
    const price = Number(rawPrice) / 1e18;

    console.log(`✅ ETH价格: $${price.toLocaleString()}`);
    console.log(`  时间戳: ${new Date(Number(timestamp) * 1000).toISOString()}`);
  } catch (error) {
    console.log(`❌ 请求失败: ${error instanceof Error ? error.message : String(error)}`);
  }
}

async function main() {
  await testTellorContract();
  await testETHPrice();
  console.log('\n=== 测试完成 ===');
}

main().catch(console.error);
