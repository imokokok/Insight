...
export const verifyExecutionPairTool: McpToolDefinition<typeof VerifyExecutionPairInputSchema> = {
  ...
  description:
    'Verify that a pre-trade oracle-safety attestation and an Execution Receipt describe the SAME authorized action and that the certify → execute → prove loop closed. Returns pairedValid, the closed-loop status (CLOSED_FAITHFUL / CLOSED_DEVIATED / CLOSED_NOT_EXECUTED / CLOSED_UNDETERMINED / PAIR_INVALID), and the binding assertions (preTradeUid, requestHash, chain, asset). This is the third-party proof a principal needs to trust an autonomous agent — pair it with agent_begin_trade and execution_receipt.',
  ...
  handler: async (args) => {
    ...
    const lines = [
      ...
      `- Binding: preTradeUid=${result.binding.preTradeUidMatch}, requestHash=${result.binding.requestHashMatch}, chain=${result.binding.chainMatch}, asset=${result.binding.assetMatch}`,
      ...
      '',
      'Note: CLOSED_FAITHFUL means the agent filled within the certified band AND the receipt is cryptographically bound to a valid pre-trade gate. This proves fidelity to the CERTIFIED price; it does not assert the price was "correct" or the trade well-timed (verification != endorsement).',
    ];
