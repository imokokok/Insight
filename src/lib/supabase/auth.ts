...
export async function resetPassword(
  ...
  const baseRedirect = `${window.location.origin}/auth/reset-password`;
  const { error } = await supabase.auth.resetPasswordForEmail(email, {
    redirectTo: redirectTo
      ? `${baseRedirect}?redirect=${encodeURIComponent(redirectTo)}`
      : baseRedirect,
  });
...
export async function resendVerification(
  ...
  const { error } = await supabase.auth.resend({
    ...
    email: email.trim(),
    options: {
      emailRedirectTo: redirectTo ?? `${window.location.origin}/auth/verify-email`,
    },
  });
