...
const PriceDataSchemaRaw = PriceDataBaseSchema.extend({
  ...
  numOracles: z.number().nullish(),
  verificationLevel: z.enum(['signed', 'unsigned']).nullish(),
  countsTowardOracleQuorum: z.boolean().nullish(),
  ...
  verification: z
    .object({
      ...
      oraclePubkey: z.string().nullish(),
      ethAddress: z.string().nullish(),
      feedHash: z.string().nullish(),
      rawValue: z.string().nullish(),
      sourceTimestamp: z.number().nullish(),
      signatureScheme: z.enum(['ed25519', 'secp256k1']).nullish(),
      recoveryId: z.number().nullish(),
    })
    .nullish(),
});
