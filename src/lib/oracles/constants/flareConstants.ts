...
export async function getFlareFeedIdAsync(symbol: string, network: string): Promise<string | null> {
  ...
  const hardcodedId = FLARE_SYMBOL_TO_FEED_ID[upper];
  if (hardcodedId) {
    return hardcodedId;
  }
...
export async function isFlareSymbolSupportedAsync(symbol: string): Promise<boolean> {
  ...
  // Hardcoded fallback when database is unavailable.
  if (flareSymbols.includes(upper as (typeof flareSymbols)[number])) {
    return true;
  }
