...
const mockClientGetHistoricalPrices = jest.fn();
const mockClientClearCache = jest.fn();
const mockClientIsSymbolSupported = jest.fn(() => true);
const mockClientGetSupportedSymbols = jest.fn(() => [mockSymbol]);
const mockClientGetDefaultChain = jest.fn(() => mockChain);
const mockGetClient = jest.fn(() => ({
  getPrice: mockClientGetPrice,
  getHistoricalPrices: mockClientGetHistoricalPrices,
  clearCache: mockClientClearCache,
  isSymbolSupported: mockClientIsSymbolSupported,
  getSupportedSymbols: mockClientGetSupportedSymbols,
...
describe('databaseOperations', () => {
  ...
  beforeEach(() => {
    ...
    mockClientGetPrice.mockReset().mockRejectedValue(new Error('Live fetch disabled in tests'));
    mockClientGetHistoricalPrices
      .mockReset()
      .mockRejectedValue(new Error('Live fetch disabled in tests'));
    mockClientClearCache.mockReset();
