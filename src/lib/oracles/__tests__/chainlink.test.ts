...
const mockIsPriceFeedSupported = jest.fn();
const mockGetChainlinkPriceFeed = jest.fn();

jest.mock('@/lib/oracles/services/chainlinkDataSources', () => ({
  isPriceFeedSupported: (...args: unknown[]) => mockIsPriceFeedSupported(...args),
  getChainlinkPriceFeed: (...args: unknown[]) => mockGetChainlinkPriceFeed(...args),
}));
...
describe('ChainlinkClient', () => {
  ...
  beforeEach(() => {
    ...
    mockIsPriceFeedSupported.mockImplementation((symbol: string, chainId: number) => {
    ...
    mockGetChainlinkPriceFeed.mockReturnValue(null);
  });
  ...
  describe('getPrice', () => {
    ...
    it('should return price data for specific chain', async () => {
      ...
      expect(chainlinkOnChainService.getPrice).toHaveBeenCalledWith('ETH', 42161, undefined);
    });
  ...
  describe('getChainId', () => {
    it('should return correct chain ID for Ethereum', async () => {
      ...
      expect(chainlinkOnChainService.getPrice).toHaveBeenCalledWith('ETH', 1, undefined);
    });
    ...
    it('should return correct chain ID for Polygon', async () => {
      ...
      expect(chainlinkOnChainService.getPrice).toHaveBeenCalledWith('ETH', 137, undefined);
    });
  ...
  describe('Boundary Conditions', () => {
    ...
    describe('Zero price handling', () => {
      it('should reject zero price value', async () => {
        const mockData = {
          symbol: 'ETH',
          price: 0,
          timestamp: Date.now(),
    ...
    describe('Negative price rejection', () => {
      it('should reject negative price value from service', async () => {
        const mockData = {
          symbol: 'ETH',
          price: -100,
          timestamp: Date.now(),
      ...
      it('should reject negative infinity price', async () => {
        const mockData = {
          symbol: 'ETH',
          price: -Infinity,
          timestamp: Date.now(),
  ...
  describe('Error Recovery Tests', () => {
    describe('Network timeout recovery', () => {
      ...
      it('should handle multiple consecutive timeouts', async () => {
        const mockGetPrice = chainlinkOnChainService.getPrice as jest.Mock;
        let callCount = 0;
        mockGetPrice.mockImplementation(() => {
          callCount += 1;
          if (callCount <= 6) {
            return Promise.reject(new Error(`Timeout ${callCount}`));
          }
