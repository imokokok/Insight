...
export function formatTooltipValue(value: number, type: 'time' | 'percentage' | 'number' | 'currency'): string {
  switch (type) {
    case 'time':
...
export function getColorForValue(value: number, thresholds: { min: number; max: number; colors: string[] }): string {
...
export function calculateChartMargins(containerWidth: number): { top: number; right: number; bottom: number; left: number } {
