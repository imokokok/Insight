...
describe('createApiHandler - basic functionality', () => {
  it('should create a handler function', () => {
    const mockHandler: ApiHandler = jest
      .fn()
  ...
  it('should use logging middleware for request ID when configured', async () => {
    const mockHandler: ApiHandler = jest
      .fn()
      .mockResolvedValue(NextResponse.json({ success: true }));
    createLoggingMiddleware.mockReturnValue(
...
describe('createApiHandler - auth middleware', () => {
...
describe('createApiHandler - validation middleware', () => {
  it('should apply validation middleware when configured', async () => {
    ...
    expect(createValidationMiddleware).toHaveBeenCalledWith({
...
describe('createApiHandler - rate limit middleware', () => {
  it('should apply rate limit middleware when configured', async () => {
    const mockHandler: ApiHandler = jest
      .fn()
      .mockResolvedValue(NextResponse.json({ success: true }));
    createRateLimitMiddleware.mockReturnValue(
      jest.fn().mockResolvedValue({
        success: true,
      })
    );
...
describe('createApiHandler - error handling', () => {
  ...
  it('should log response on success', async () => {
    ...
    expect(logResponse).toHaveBeenCalledWith(expect.any(String), 200, expect.any(Number));
  });
});

describe('createApiHandler - middleware order', () => {
