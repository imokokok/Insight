...
export default function LoginPage() {
  ...
  const parseError = (errorMessage: string): ErrorInfo => {
    ...
    if (lowerError.includes('email not confirmed') || lowerError.includes('email_not_confirmed')) {
      return {
        message: t('auth.login.error.emailNotConfirmed'),
            ...
            <div className="mb-6 p-4 bg-red-50 border border-red-200">
              <div className="flex items-start gap-3">
                {errorInfo?.type === 'email_not_confirmed' ? (
                  <MailWarning className="w-5 h-5 text-red-600 flex-shrink-0 mt-0.5" />
                ) : (
          ...
          <div className="mt-6">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <button
                ...
                disabled={isLoading}
                className="flex items-center justify-center gap-2 px-4 py-2.5 border border-gray-200 text-gray-700 font-medium hover:bg-gray-50 hover:border-gray-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
