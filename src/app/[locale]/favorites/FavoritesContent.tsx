...
export default function FavoritesContent() {
  ...
  const handleApply = (config: FavoriteConfig, configType: ConfigType) => {
    switch (configType) {
      case 'oracle_config':
        router.push(
          `/${locale}/cross-oracle?oracles=${config.selectedOracles?.join(',')}&symbol=${config.symbol}`
        );
        break;
      case 'symbol':
        router.push(`/${locale}/price-query?symbol=${config.symbol}`);
        break;
      case 'chain_config':
        router.push(`/${locale}/cross-chain?chain=${config.chain}`);
        break;
    }
  };
