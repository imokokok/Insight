        ...
        {metrics.map((metric) => (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              >
                {t(`dataQuality.${SCORE_CONFIG[getScoreLevel(metric.completenessScore)].labelKey}`)}
              </span>
            </div>
...
function FreshnessTrendChart({
  ...
  const getLevelLabel = (level: ScoreLevel) => {
    return t(`dataQuality.${SCORE_CONFIG[level].labelKey}`);
  };
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-gray-500">
            {t('dataQuality.lastUpdated')}: {lastUpdated.toLocaleTimeString()}
          </span>
          <button
