...
const mockMarketData = {
  marketCap: 280000000, // $280M
  volume24h: 12500000, // $12.5M
  circulatingSupply: 100000000, // 100M API3
  totalSupply: 125000000, // 125M API3
  fullyDilutedValuation: 350000000, // $350M
  marketCapRank: 189,
  high24h: 2.95,
  low24h: 2.65,
  change24h: 8.45, // +8.45%
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-4 text-sm">
        <div className="flex items-center gap-2">
          <span className="text-gray-500">{t('api3.marketData.24hHigh')}</span>
          <span className="text-green-600 font-medium">${high24h.toFixed(2)}</span>
        </div>
        <div className="w-px h-4 bg-gray-300" />
        <div className="flex items-center gap-2">
          <span className="text-gray-500">{t('api3.marketData.24hLow')}</span>
...
export function MarketDataPanel() {
  const { t } = useI18n();
  const [price, setPrice] = useState<number>(2.8);
  const [previousPrice, setPreviousPrice] = useState<number>(2.8);
  ...
  const fetchPrice = useCallback(async () => {
    ...
    try {
      const priceData = await api3Client.getPrice('API3', Blockchain.ETHEREUM);
      ...
      console.error('Error fetching API3 price:', error);
    } finally {
  ...
  const metrics = [
    {
      label: t('api3.marketData.marketCap'),
    ...
    {
      label: t('api3.marketData.volume'),
    ...
    {
      label: t('api3.marketData.circulatingSupply'),
      value: `${formatNumber(mockMarketData.circulatingSupply, true)} API3`,
    ...
    {
      label: t('api3.marketData.rank'),
      value: `#${mockMarketData.marketCapRank}`,
      subValue: t('api3.marketData.amongAllCryptocurrencies'),
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            strokeLinecap="round"
    ...
    {
      label: t('api3.marketData.supplyRatio'),
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6">
        <div className="flex items-center justify-center h-48">
          <div className="flex items-center gap-2 text-gray-400">
            ...
            <span>{t('api3.marketData.loading')}</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6">
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 bg-blue-700 rounded-xl flex items-center justify-center shadow-lg shadow-blue-700/25">
            <span className="text-white font-bold text-xl">API3</span>
          </div>
          <div>
            <h2 className="text-gray-900 font-semibold text-lg">API3</h2>
            <p className="text-gray-500 text-sm">API3 / USD</p>
          </div>
        </div>
        <div className="text-right">
          <p className="text-gray-400 text-xs">{t('api3.lastUpdated')}</p>
