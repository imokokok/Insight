  ...
  recentEvents: [
    {
      date: '2024-01-15',
      type: 'upgrade',
      description: 'Smart contract upgrade v2.1',
      status: 'completed',
    },
    {
      date: '2024-01-10',
      type: 'audit',
      description: 'Security audit by Trail of Bits',
      status: 'completed',
    },
    {
      date: '2024-01-05',
      type: 'governance',
      description: 'DAO proposal #42 passed',
      status: 'completed',
    },
  ],
};
      ...
      <div className={`bg-white border ${overallConfig.borderColor} rounded-xl p-6`}>
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          <div className={`p-4 rounded-xl ${overallConfig.bgColor}`}>
            <svg
              className={`w-12 h-12 ${overallConfig.color}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div className="flex-1">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">
              {t('api3.risk.overallAssessment')}
            </h3>
            <div className="flex items-center gap-3 mb-2">
              <span
                className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${overallConfig.bgColor} ${overallConfig.color}`}
              >
                {t(`api3.risk.${riskData.overallRisk}Risk`)}
              </span>
              <span className="text-sm text-gray-500">
                {t('api3.risk.lastUpdated')}: {t('api3.justNow')}
              </span>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {riskData.mitigations.map((measure, index) => (
            <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
              <div
                className={`w-2 h-2 rounded-full ${measure.status === 'active' ? 'bg-green-500' : 'bg-yellow-500'}`}
              />
              <div className="flex-1">
              ...
              <span
                className={`text-xs px-2 py-0.5 rounded ${measure.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}
              >
                {measure.status}
              </span>
            </div>
