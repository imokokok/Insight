...
const mockMarketData = {
  ...
  circulatingSupply: 1500000000, // 1.5B PYTH
  totalSupply: 10000000000, // 10B PYTH
  fullyDilutedValuation: 8330000000, // $8.33B
  marketCapRank: 78,
  high24h: 0.8523,
  low24h: 0.7891,
  change24h: 3.45, // +3.45%
  change24hValue: 0.0285,
  stakingApy: 8.5,
  stakedAmount: 450000000, // 450M PYTH
};
 ...
 * 格式化货币显示
 * @param value - 数值
 * @param compact - 是否使用紧凑格式（如 1.25B）
 */
...
function StakingCard({
  ...
  const stakingRatio = (stakedAmount / circulatingSupply) * 100;

  return (
    <div className="bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-200 rounded-xl p-4">
      <div className="flex items-start justify-between mb-3">
        <div>
          <p className="text-purple-700 text-xs uppercase tracking-wider mb-1">Staking</p>
          <p className="text-purple-900 text-lg font-semibold">{stakingApy.toFixed(1)}% APY</p>
        </div>
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-xs">
          ...
          <span className="text-purple-900 font-medium">{stakingRatio.toFixed(1)}%</span>
        </div>
        <div className="h-2 bg-purple-200 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-purple-500 to-indigo-500 rounded-full transition-all duration-500"
            style={{ width: `${Math.min(stakingRatio, 100)}%` }}
          />
        </div>
        <p className="text-purple-600 text-xs">
          {formatNumber(stakedAmount, true)} / {formatNumber(circulatingSupply, true)} PYTH
        </p>
      </div>
    </div>
  );
...
export function MarketDataPanel() {
  const [price, setPrice] = useState<number>(0.8234);
  const [previousPrice, setPreviousPrice] = useState<number>(0.8234);
  ...
  const fetchPrice = useCallback(async () => {
    ...
    try {
      const priceData = await pythClient.getPrice('PYTH', Blockchain.SOLANA);
  ...
  const metrics = [
    ...
    },
    {
      ...
      value: `${formatNumber(mockMarketData.circulatingSupply, true)} PYTH`,
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6">
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 bg-gradient-to-br from-purple-600 to-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-purple-600/25">
            <span className="text-white font-bold text-xl">P</span>
          </div>
          <div>
            <h2 className="text-gray-900 font-semibold text-lg">Pyth Network</h2>
            <p className="text-gray-500 text-sm">PYTH / USD</p>
          </div>
        </div>
      ...
      {/* 质押数据卡片 */}
      <StakingCard
        stakedAmount={mockMarketData.stakedAmount}
        stakingApy={mockMarketData.stakingApy}
        circulatingSupply={mockMarketData.circulatingSupply}
      />
    </div>
  );
}
