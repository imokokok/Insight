...
function ActivityHeatmap({ hourlyData }: { hourlyData: number[] }) {
  ...
  const getIntensity = (value: number) => {
    ...
    if (ratio > 0.8) return 'bg-purple-400';
    if (ratio > 0.6) return 'bg-purple-500';
    if (ratio > 0.4) return 'bg-purple-600';
    if (ratio > 0.2) return 'bg-purple-700';
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-gray-900 text-sm font-semibold">数据验证活动热力图</p>
          <p className="text-gray-500 text-xs mt-0.5">24小时验证请求分布</p>
        </div>
      ...
      <div className="grid grid-cols-12 gap-1.5">
        {hourlyData.map((value, index) => (
          <div key={index} className="group relative">
            <div
              className={`h-10 rounded-md ${getIntensity(value)} transition-all duration-300 hover:scale-110 hover:ring-2 hover:ring-purple-300 cursor-pointer`}
              title={`${getHourLabel(index)}: ${value.toLocaleString()} 验证`}
            />
            {/* 悬停提示 */}
      ...
      <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
        <div className="text-center">
          <p className="text-xs text-gray-500">总验证量</p>
        ...
        <div className="text-center">
          <p className="text-xs text-gray-500">峰值验证</p>
  ...
  const metrics: MetricCard[] = [
    {
      id: 'activeValidators',
      title: '活跃验证者数',
      value: `${networkData.activeValidators.toLocaleString()}+`,
      trend: 3.2,
      trendDirection: 'up',
      icon: (
    ...
    },
    {
      id: 'validatorUptime',
      title: '验证者在线率',
      value: networkData.validatorUptime.toFixed(1),
      unit: '%',
      trend: 0.2,
      trendDirection: 'up',
      icon: (
    ...
    },
    {
      id: 'dataSources',
      title: '数据源数量',
      value: `${networkData.dataSources.toLocaleString()}+`,
      trend: 2.1,
      trendDirection: 'up',
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
