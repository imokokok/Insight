...
type ExchangeName = 'Binance' | 'Coinbase' | 'Kraken' | 'Uniswap';
...
const mockQualityData: QualityData = {
  chainlinkPrice: 145.82,
  exchangePrices: [
    {
      name: 'Binance',
      price: 145.91,
      deviationPercent: 0.06,
      deviationAmount: 0.09,
      status: 'normal',
    },
    {
      name: 'Coinbase',
    ...
    },
    {
      ...
      deviationAmount: 0.73,
      status: 'critical',
    },
  ],
  latencyDistribution: [
    { range: '<100ms', count: 2458, color: '#10b981' },
    { range: '100-300ms', count: 1823, color: '#3b82f6' },
    { range: '300-500ms', count: 567, color: '#f59e0b' },
    { range: '500-1000ms', count: 234, color: '#f97316' },
    { range: '>1000ms', count: 89, color: '#ef4444' },
  ],
  anomalyData: {
    totalAnomalies: 23,
    anomalyRate: 0.42,
    types: [
      { type: '价格跳跃', count: 12, percentage: 52.2, color: '#ef4444' },
      { type: '延迟过高', count: 7, percentage: 30.4, color: '#f59e0b' },
      { type: '数据缺失', count: 4, percentage: 17.4, color: '#3b82f6' },
    ],
  },
  consistencyScore: {
    score: 87,
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-5">
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="mt-5 w-full space-y-2">
          {[
            { label: '价格准确性', value: 92 },
            { label: '数据及时性', value: 85 },
            { label: '来源多样性', value: 88 },
            { label: '异常检测', value: 83 },
          ].map((item) => (
  ...
  const simulateDataUpdate = useCallback(() => {
    setQualityData((prev) => {
      ...
      const updatedExchanges = prev.exchangePrices.map((exchange) => {
        ...
        const deviationPercent = ((newPrice - prev.chainlinkPrice) / prev.chainlinkPrice) * 100;
        const deviationAmount = newPrice - prev.chainlinkPrice;
          ...
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center">
              <svg className="w-6 h-6 text-white" viewBox="0 0 256 256" fill="currentColor">
                <path d="M128 0L16 64v128l112 64 112-64V64L128 0z" fillOpacity="0.3" />
                <path d="M208 64l-80 46-80-46 80-46 80 46z" fillOpacity="0.5" />
                <path d="M48 64v128l80 46V110l-80-46z" fillOpacity="0.7" />
                <path d="M208 64v128l-80 46V110l80-46z" />
              </svg>
            </div>
            <div>
