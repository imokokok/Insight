...
function ErrorItemRow({ error, onRetry, isRetrying }: ErrorItemRowProps) {
  ...
  return (
    <div className="flex items-center justify-between py-3 px-4 hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-b-0">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <div className={`border border-current p-2 ${errorInfo.bgColor}`}>
          <span className={errorInfo.color}>{errorInfo.icon}</span>
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-medium text-slate-900 text-sm">{error.provider}</span>
            <span className="text-xs text-slate-400">•</span>
            <span className="text-xs text-slate-500">{error.chain}</span>
          </div>
          <div className="flex items-center gap-2 mt-1">
            <span className={`text-xs font-medium ${errorInfo.color}`}>{errorInfo.label}</span>
            <span className="text-xs text-slate-400 truncate max-w-[200px]" title={error.error}>
              {error.error}
            </span>
          </div>
        </div>
...
export function ErrorBanner({ errors, onRetry, onRetryAll, onDismiss }: ErrorBannerProps) {
  ...
  return (
    <div className="overflow-hidden border-l-2 border-red-500 bg-red-50">
      ...
      {hasMoreErrors && (
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 py-2.5 text-xs font-medium text-slate-500 hover:text-slate-700 hover:bg-slate-50 transition-colors border-t border-slate-200"
