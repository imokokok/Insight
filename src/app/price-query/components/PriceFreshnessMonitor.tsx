  ...
  if (dataSources.length === 0) {
    return (
      <div className="h-[200px] flex flex-col items-center justify-center text-slate-400">
        ...
        <p className="text-xs mt-1 text-slate-500">Query prices to see data freshness analysis</p>
      </div>
    );
  }
            ...
            <div className="flex-1 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-medium text-slate-700">Data Freshness Distribution</p>
                </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200">
              <th className="text-left py-2 px-3 font-medium text-slate-500 text-xs">Oracle</th>
              <th className="text-right py-2 px-3 font-medium text-slate-500 text-xs">Price</th>
              {hasMultipleSources && (
                <th className="text-right py-2 px-3 font-medium text-slate-500 text-xs">
                  Deviation
                </th>
              )}
              <th className="text-center py-2 px-3 font-medium text-slate-500 text-xs">
                Freshness
              </th>
              <th className="text-center py-2 px-3 font-medium text-slate-500 text-xs">Rhythm</th>
              <th className="text-center py-2 px-3 font-medium text-slate-500 text-xs">
                Reliability
              </th>
              <th className="text-center py-2 px-3 font-medium text-slate-500 text-xs">Health</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {dataSources.map((source) => {
              ...
              return (
                <tr key={source.key} className="hover:bg-slate-50 transition-colors">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                          <p className="font-medium text-slate-900">{source.provider}</p>
                          {source.isRealtime && (
                  ...
                  <td className="py-2.5 px-3">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-[10px] text-slate-400 mt-0.5">{grade.label}</span>
                    </div>
                  </td>
                </tr>
              );
