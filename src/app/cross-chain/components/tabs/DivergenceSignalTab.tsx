...
function getChainDisplayName(chainKey: string): string {
  return (chainNames as Record<string, string>)[chainKey] || capitalize(chainKey);
}
...
function DivergenceSignalTabComponent({ divergence, feed }: DivergenceSignalTabProps) {
  const sortedLeadership = [...divergence.leadership].sort((a, b) => a.lagSeconds - b.lagSeconds);

  const matrixChainNames =
    divergence.divergenceMatrix.length > 0
      ? divergence.divergenceMatrix.map((row) => row[0]?.providerA ?? '')
      : [];
    ...
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-medium text-gray-700">
              Cross-Chain Divergence Signals
            </span>
          </div>
          <p className="text-xs text-gray-500">
            Track how each chain deviates from cross-chain consensus over time, detect accelerating
            deviations and directional bias
          </p>
        </div>
        {divergence.alertCount > 0 && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-amber-50 border border-amber-200">
            <AlertTriangle className="w-3.5 h-3.5 text-amber-600" />
            <span className="text-xs font-semibold text-amber-700">
              {divergence.alertCount} Alert{divergence.alertCount !== 1 ? 's' : ''}
            </span>
          </div>
        )}
      </div>

      {divergence.timeSeries.length === 0 && (
        <div className="bg-blue-50 rounded-xl border border-blue-200 p-6 text-center">
          ...
          <p className="text-xs text-blue-500 mt-1">
            Divergence signals require at least 2 chains with price history. Enable auto-refresh to
            accumulate data points over time.
          </p>
        </div>
      )}

      {divergence.timeSeries.length > 0 &&
        divergence.timeSeries.every((ts) => ts.points.length < 3) && (
          <div className="bg-amber-50 rounded-xl border border-amber-200 p-4 text-center">
            <p className="text-xs text-amber-700">
              Limited data available — acceleration and directional bias detection require 3+ data
              points per chain. Keep auto-refresh enabled to accumulate more data.
            </p>
          </div>
        )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <p className="text-2xl font-bold text-gray-900 font-mono">{divergence.alertCount}</p>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-center gap-2 mb-2">
            <Zap
              className={`w-4 h-4 ${divergence.acceleratingCount > 0 ? 'text-red-500' : 'text-emerald-500'}`}
            />
            <span className="text-sm font-medium text-gray-700">Accelerating</span>
          </div>
          <p
            className={`text-2xl font-bold font-mono ${divergence.acceleratingCount > 0 ? 'text-red-600' : 'text-emerald-600'}`}
          >
            {divergence.acceleratingCount}
          </p>
          <p className="text-[10px] text-gray-400 mt-1">
            Chains with accelerating deviation
            {divergence.maxAcceleration > 0 && (
              <span className="text-red-500 ml-1">
                Max: {divergence.maxAcceleration.toFixed(4)}%/update
              </span>
            )}
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-center gap-2 mb-2">
            <Navigation
              className={`w-4 h-4 ${divergence.directionalBiasCount > 0 ? 'text-orange-500' : 'text-emerald-500'}`}
            />
            <span className="text-sm font-medium text-gray-700">Directional Bias</span>
          </div>
          <p
            className={`text-2xl font-bold font-mono ${divergence.directionalBiasCount > 0 ? 'text-orange-600' : 'text-emerald-600'}`}
          >
            {divergence.directionalBiasCount}
          </p>
          <p className="text-[10px] text-gray-400 mt-1">Chains with directional bias</p>
        </div>

        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-center gap-2 mb-2">
            <TrendingUp className="w-4 h-4 text-blue-500" />
            <span className="text-sm font-medium text-gray-700">Leading Chain</span>
          </div>
          <p className="text-2xl font-bold text-blue-600 font-mono">
            {divergence.leadingOracle ? getChainDisplayName(divergence.leadingOracle) : 'N/A'}
          </p>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-base font-semibold text-gray-900">Chain Deviation Time Series</span>
        </div>
        <div className="space-y-3">
          {divergence.timeSeries.map((ts) => {
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-gray-900">
                      {getChainDisplayName(ts.provider)}
                    </span>
                    <span
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-base font-semibold text-gray-900">Chain Response Leadership</span>
        </div>
        <div className="space-y-2">
          {sortedLeadership.map((chain, index) => {
            const badge = getLeadershipBadge(chain.status);
            const reliability =
              chain.totalUpdates > 0 ? (chain.firstResponseCount / chain.totalUpdates) * 100 : 0;
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-gray-900">
                      {getChainDisplayName(chain.provider)}
                    </span>
                    <span
                  ...
                  <div className="flex items-center gap-3 text-xs text-gray-500">
                    <span>
                      ...
                      <span className="font-mono font-medium text-gray-700">
                        {chain.avgLagSeconds.toFixed(2)}s
                      </span>
                    </span>
                    <span>
                      First response:{' '}
                      <span className="font-mono font-medium text-gray-700">
                        {chain.firstResponseCount}/{chain.totalUpdates}
                      </span>
                    </span>
                  </div>
                </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {divergence.divergenceMatrix.map((row, rowIdx) => (
                  <tr key={`row-${rowIdx}`} className="border-t border-gray-100">
                    <td className="px-2 py-1.5 text-[10px] font-medium text-gray-700 whitespace-nowrap">
                      {getChainDisplayName(matrixChainNames[rowIdx])}
                    </td>
                    {row.map((pair, colIdx) => {
                      if (colIdx === rowIdx) {
                        return (
                      ...
                      const deviation =
                        colIdx > rowIdx
                          ...
                          : (divergence.divergenceMatrix[colIdx]?.[rowIdx]?.deviationPercent ?? 0);
                      return (
      ...
      {feed.healthScores.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-center gap-2 mb-4">
            <Activity className="w-5 h-5 text-gray-700" />
            <span className="text-base font-semibold text-gray-900">Chain Feed Health Summary</span>
          </div>
          <div className="space-y-3">
            {feed.healthScores.map((chain) => {
              const badge = getHealthBadge(chain.score);
              return (
                <div key={chain.provider} className="border border-gray-100 rounded-lg p-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-lg font-bold text-gray-900 font-mono">{chain.score}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[10px] text-gray-500">
                    <span>
                      Rhythm:{' '}
                      <span className="font-mono font-medium text-gray-700">
                        {chain.rhythmStability}%
                      </span>
                    </span>
                    <span>
                      Confidence:{' '}
                      <span className="font-mono font-medium text-gray-700">
                        {chain.confidenceStability}%
                      </span>
                    </span>
                    <span>
                      Heartbeat:{' '}
                      <span className="font-mono font-medium text-gray-700">
                        {chain.heartbeatReliability}%
                      </span>
                    </span>
                    <span>
                      Freshness:{' '}
