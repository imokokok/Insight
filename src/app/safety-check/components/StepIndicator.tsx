          ...
          {PHASES.map((phase, index) => {
            ...
            return (
              <div key={phase.id} className="flex flex-col items-center flex-1">
                ...
                <span
                  className={cn(
                    'text-[10px] mt-1.5 font-semibold text-center transition-colors duration-300 leading-tight',
                    isCompleted && 'text-blue-600',
                    (isCurrent || isError) && 'text-slate-900',
                    !isCompleted && !isCurrent && !isError && 'text-slate-400'
                  )}
                >
                  {phase.label}
                </span>
...
function StatusBadge({
  ...
  return <span className="text-xs font-semibold text-slate-400">Ready</span>;
}
