...
import { formatDateTimeString, formatNumberWithDecimals, getNestedValue } from '@/lib/utils/format';
...
/**
 * Trim export history items by max count and optional max age.
 * Applies the item cap first, then filters out expired items.
 */
export function trimHistoryItems(
  ...
  let filtered = items.slice(0, maxItems);
...
function saveExportHistory(item: ExportHistoryItem): void {
  ...
  try {
    ...
    const maxAgeMs = settings.autoCleanup
      ? settings.cleanupAfterDays * 24 * 60 * 60 * 1000
      : undefined;
    const filtered = trimHistoryItems(updated, settings.maxHistoryItems, maxAgeMs);
