...
export default function ComparisonDashboard() {
  ...
  return (
    <div className="space-y-8">
      <section>
        <h2>时间段对比</h2>
        <TimeRangeSelector value={timeConfig} onChange={setTimeConfig} />
      </section>

      <section>
        <h2>预言机对比</h2>
        <OracleComparisonView oracles={oracleData} showCharts={true} showTable={true} />
      </section>

      <section>
        <h2>行业基准对比</h2>
        <BenchmarkComparison actualValue={65000} benchmarkType="industry_average" />
      </section>
    </div>
  );
}
