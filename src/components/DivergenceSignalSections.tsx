...
export const LABELS: Record<DivergenceMode, Record<string, string>> = {
  ...
  oracle: {
    ...
    subtitle:
      'Track how each oracle deviates from market consensus over time, detect accelerating deviations and directional bias. Based on accumulated polled data (up to 24h window).',
    emptyMessage:
        ...
        <p className="text-[10px] text-gray-400 mt-1">
          ...
          {maxAcceleration > 0 && (
            <span className="text-red-500 ml-1">
              Max rate: {maxAcceleration.toFixed(4)}%/update
            </span>
          )}
        </p>
      </div>
