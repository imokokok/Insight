...
export function FilterPanel({
  ...
  return (
    <div ref={panelRef} className={`bg-white border border-gray-200  ${className}`}>
      {showSearch && (
        <div className="p-3 border-b border-gray-200">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 text-sm border border-gray-200  focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
        </div>
      )}
          ...
          <div className="flex flex-wrap gap-2">
            {activeFilterTags.map((tag) => (
              <motion.span
                ...
                className="inline-flex items-center gap-1 px-2 py-1 bg-blue-100 text-blue-700 text-xs "
              >
                <span className="font-medium">{tag.label}</span>
                <button
                  onClick={() => handleOptionToggle(tag.sectionId, tag.value, true)}
                  className="p-0.5 hover:bg-blue-200  transition-colors"
              ...
              <div className="flex items-center gap-2">
                {filters[section.id]?.length > 0 && (
                  <motion.span
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="px-2 py-0.5 bg-blue-100 text-blue-600 text-xs "
                  >
                        ...
                        return (
                          ...
                          >
                            <input
                              ...
                              onKeyDown={(e) =>
                                handleKeyDown(e, section.id, optionIndex, section.options.length)
                              }
      ...
      {hasActiveFilters && (
        <div className="p-3 border-t border-gray-200 bg-gray-50 -lg">
          <button
            onClick={handleClearAll}
            className="w-full px-4 py-2 text-sm text-gray-600 hover:text-gray-900 hover:bg-gray-100  transition-colors"
          >
            清除所有筛选
          </button>
        </div>
