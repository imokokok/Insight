      ...
      <DashboardCard title={t('winklink.staking.title')}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-gray-900">{formatNumber(data.totalStaked)} WIN</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-2xl font-bold text-green-600">{data.activeNodes}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-2xl font-bold text-yellow-600">{data.averageApr}%</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-2xl font-bold text-gray-900">{formatNumber(data.rewardPool)} WIN</p>
          </div>
        </div>
      </DashboardCard>
      ...
      <DashboardCard title={t('winklink.staking.tiers')}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {data.stakingTiers.map((tier, index) => (
            <div
              key={index}
              className={`p-4 rounded-lg border-2 text-center ${getTierColor(tier.tier.toLowerCase())}`}
            >
              <h4 className="font-bold text-lg capitalize mb-2">{tier.tier}</h4>
              <div className={`w-12 h-12 mx-auto mb-3 rounded-full bg-gradient-to-br ${getTierGradient(tier.tier.toLowerCase())} flex items-center justify-center`}>
                <Award className="w-6 h-6 text-white" />
              </div>
              <p className="text-3xl font-bold mb-1">{tier.apr}%</p>
              <p className="text-sm opacity-75">APR</p>
            ...
            <tbody>
              {data.nodes.slice(0, 5).map((node, index) => (
                <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4">
                    <span className={`px-2 py-1 rounded text-xs font-medium capitalize ${getTierColor(node.tier)}`}>
                      {node.tier}
                    </span>
                  </td>
                  <td className="py-3 px-4 text-gray-900">{formatNumber(node.stakedAmount)}</td>
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-1">
                  ...
                  <td className="py-3 px-4 text-green-600">{formatNumber(node.rewardsEarned)}</td>
                </tr>
              ))}
            </tbody>
          </table>
