      ...
      <DashboardCard title={t('chronicle.makerdao.title')}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-gray-900">{formatCurrency(data.totalValueLocked)}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-2xl font-bold text-gray-900">{formatCurrency(data.daiSupply)}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-2xl font-bold text-green-600">{formatCurrency(data.systemSurplus)}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-2xl font-bold text-gray-900">{formatCurrency(data.globalDebtCeiling)}</p>
          </div>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="text-left py-3 px-4 text-sm font-medium text-gray-500">{t('chronicle.makerdao.debtCeiling')}</th>
              </tr>
            </thead>
            <tbody>
              {data.supportedAssets.map((asset, index) => (
                <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4">
                    <span className={`px-2 py-1 rounded text-xs font-medium capitalize ${getAssetTypeColor(asset.type)}`}>
                      {asset.type}
                    </span>
                  </td>
