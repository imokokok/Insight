...
import type { AlertEvent } from '@/lib/supabase/queries';
...
export function useRealtimeAlerts(options: UseRealtimeAlertsOptions = {}): UseRealtimeAlertsReturn {
  ...
  const { connectionStatus, subscribeToAlertEvents } = useRealtime();
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToAlertEvents(handleAlertEvent);

    return () => {
      unsubscribe();
    };
  }, [enabled, handleAlertEvent, subscribeToAlertEvents]);
