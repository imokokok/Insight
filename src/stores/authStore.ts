...
const fetchUserProfile = async (userId: string, session: Session | null) => {
  try {
    ...
    // Only attempt to create a profile when the fetch fails because the
    // profile does not exist (404).  Network errors, 500s, etc. should not
    // trigger profile creation — that would create duplicate records.
    const isNotFound =
    ...
    if (!isNotFound) {
      logger.error(
        'Failed to fetch user profile (non-404 error, not attempting creation)',
    ...
    try {
      ...
      logger.error(
        'Failed to create user profile after 404',
      ...
      (set, get) => ({
        ...
        signUp: async (
          ...
          const {
            ...
            session: newSession,
            error: signUpError,
          } = await authSignUp(email, password, displayName, redirectTo);
        ...
        resetPassword: async (email: string, redirectTo?: string) => {
          ...
          const { error: resetError } = await authResetPassword(email, redirectTo);
        ...
        resendVerification: async (email: string, redirectTo?: string) => {
          set({ loading: true, error: null });

          const { error: resendError } = await authResendVerification(email, redirectTo);
